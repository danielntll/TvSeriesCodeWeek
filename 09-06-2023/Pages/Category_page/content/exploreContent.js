...
export const renderExploreContent = (title, callbackFetch) => {
  ...
  loadData(category_content_grid, callbackFetch);

  return category_content;
};

const loadData = async (fatherToAppend, callbackFetch) => {
  let currentIndexPage = 1;
  let loadingData = false;
  let result = await callbackFetch(currentIndexPage);
  ...
  if (fatherToAppend.offsetHeight < window.innerHeight) {
    fillPage(fatherToAppend, currentIndexPage, (page) => callbackFetch(page));
  }
  ...
  onscroll = async () => {
    if (
      ...
      let newResult = await callbackFetch(currentIndexPage);
      setTimeout(() => {
...
const fillPage = async (fatherToAppend, currentIndexPage, callbackFetch) => {
